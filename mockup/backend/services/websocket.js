...
class WebSocketService {
  ...
  initialize(server, corsOrigin) {
    ...
    this.io.on('connection', socket => {
      ...
      // Join game chat room
      socket.on('join-game-chat', data => {
        const { gameId } = data;
        socket.join(`game-chat-${gameId}`);

        // Store socket reference for route handlers
        socket.request.socket = socket;

        console.log(`Socket ${socket.id} joined game chat: ${gameId}`);
        socket.emit('joined-game-chat', { gameId });
      });

      // Leave game chat room
      socket.on('leave-game-chat', data => {
        const { gameId } = data;
        socket.leave(`game-chat-${gameId}`);

        console.log(`Socket ${socket.id} left game chat: ${gameId}`);
        socket.emit('left-game-chat', { gameId });
      });

      // Handle typing indicators for game chat
      socket.on('game-chat-typing', data => {
        const { gameId, userName } = data;
        socket.to(`game-chat-${gameId}`).emit('user-typing', {
          userId: socket.id,
          userName,
          gameId
        });
      ...
      socket.on('game-chat-stop-typing', data => {
        const { gameId } = data;
        socket.to(`game-chat-${gameId}`).emit('user-stopped-typing', {
          userId: socket.id,
          gameId
        });
      });
