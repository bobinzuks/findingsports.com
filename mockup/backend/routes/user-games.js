...
router.post('/submit', authenticateToken, async (req, res) => {
  try {
    ...
    const gameForChat = {
      ...
      dateTime: storedGame.startTime,
      endTime: storedGame.endTime || new Date(new Date(storedGame.startTime).getTime() + 2 * 60 * 60 * 1000)
    };

    const chatRoom = await chatRoomService.createGameChatRoom(gameForChat, user.id);
